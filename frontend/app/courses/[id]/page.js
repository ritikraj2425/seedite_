...
import { ChevronDown, ChevronRight, X, ArrowLeft, CheckCircle } from 'lucide-react';
        ...
        const fetchCourse = async () => {
            ...
            try {
                ...
                if (profileRes.ok) {
                    ...
                    // Track attempted tests
                    if (profileData.mockTestResults) {
                        const attempted = new Set();
                        profileData.mockTestResults.forEach(result => {
                            if (result.test) {
                                attempted.add(result.test._id ? result.test._id.toString() : result.test.toString());
                            }
                        });
                ...
                setIsEnrolled(userEnrolled || data.hasCollegeAccess === true);

                if (userEnrolled || data.hasCollegeAccess === true) {
                    fetch(`${API_URL}/api/progress/${id}`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    }).then(r => r.ok ? r.json() : null).then(progressData => {
                        if (progressData?.completedLectures) {
                            setCompletedLectures(new Set(progressData.completedLectures.map(pid => pid.toString())));
                        }
                                                                    ...
                                                                    <Card style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                        <div style={{ display: 'flex', alignItems: 'center' }}>
                                                                            {completedLectures.has(lecture._id?.toString()) ? (
                                                                                <CheckCircle size={18} color="#22c55e" style={{ marginRight: '8px', flexShrink: 0 }} />
                                                                            ) : (
                                            ...
                                            {course?.mockTests?.map((test) => (
                                                <Card key={test._id} style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    ...
                                                    <Link href={`/courses/${id}/mock-test/${test._id}`}>
                                                        <Button style={{ fontSize: '0.9rem', padding: '8px 12px', width: '120px', justifyContent: 'center' }}>
                                                            {attemptedTests.has(test._id.toString()) ? 'View Result' : 'Start Test'}
                                                        </Button>
                                                    </Link>
                                                </Card>
                                            ))}
                        ...
                        <div style={{ position: 'sticky', top: '100px', maxHeight: 'calc(100vh - 120px)', overflowY: 'auto', paddingRight: '4px' }} className="custom-scrollbar">
                            <Card style={{ padding: '0', overflow: 'hidden', marginBottom: '24px' }}>
                                ...
                                <div style={{ padding: '24px' }}>
                                    {!isEnrolled ?
                                        ...
                                        <div style={{ marginBottom: '16px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                                ...
                                                <span style={{ fontWeight: 600, color: '#2563eb', fontSize: '0.9rem' }}>{progressPercentage}%</span>
                                            </div>
                                            <div style={{ width: '100%', height: '6px', background: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                                                <div style={{ height: '100%', background: 'linear-gradient(90deg, #3b82f6, #2563eb)', width: `${progressPercentage}%`, transition: 'width 0.4s ease', borderRadius: '4px' }}></div>
                                            </div>
                                        </div>}
                                    ...
                                    {isEnrolled && (
                                        <a
                                            href={course.communityLink || course.telegramUrl || "https://t.me/+xiJsaZjnDf02YWY1"}
                                            target="_blank"
                                        ...
                                        >
                                            <Button
                                                style={{
                                                    width: '100%',
                                                    marginBottom: '16px',
                                                    background: course.communityType === 'whatsapp' ? '#25D366' : '#0088cc',
                                                    display: 'flex',
                                                    alignItems: 'center',
                                            ...
                                            >
                                                ...
                                                {course.communityType === 'whatsapp' ? 'Join WhatsApp Group' : 'Join Telegram Group'}
                                            </Button>
                                        </a>
                                    )}

...
export interface MockTestBuilderProps {
    mode: 'create' | 'edit';
    testType: 'course' | 'iq';
    courseId?: string;
    testId?: string;
...
export default function MockTestBuilder({ mode, testType, courseId, testId }: MockTestBuilderProps) {
    ...
    const fetchTestDetails = async () => {
        ...
        try {
            ...
            const url = `${API_URL}/api/mock-tests/${testId}`;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const endpoint = testType === 'iq' ? '/api/admin/iq-tests' : '/api/admin/mock-tests';
            const url = mode === 'edit' ? `${API_URL}${endpoint}/${testId}` : `${API_URL}${endpoint}`;
            const res = await fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    totalQuestions: parseInt(totalQuestions) || questions.length,
                    passingScore: parseInt(passingScore) || 0,
                    correctMarks: parseInt(correctMarks) || 4,
                    incorrectMarks: parseInt(incorrectMarks) || 0,
                    videoSolutionKey,
                    ...(testType === 'course' && courseId ? { courseId } : {}),
            ...
            if (res.ok) {
                toast.success(testType === 'iq' ? `IQ Test ${mode}d!` : `Mock Test ${mode}d!`);
                router.push(testType === 'iq' ? '/iq-tests' : `/courses/${courseId}`);
            } else {
                toast.error(`Failed to ${mode} test`);
            }
        } catch (error) {
            toast.error(`Error ${mode}ing test`);
        }
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between h-16 items-center">
                        <div className="flex items-center space-x-4">
                            {testType === 'iq' ? <Link href="/iq-tests" className="text-gray-400 hover:text-white">← Back</Link> : <Link href={`/courses/${courseId}`} className="text-gray-400 hover:text-white">← Back</Link>}
                            <h1 className="text-2xl font-bold">{mode === 'create' ? 'Create' : 'Edit'} {testType === 'iq' ? 'IQ Test' : 'Mock Test'}</h1>
                        </div>
                    </div>
                </div>
            </nav>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="flex justify-end pt-4">
                        ...
                        >
                            {uploading ? 'Uploading...' : `${mode === 'create' ? 'Create' : 'Update'} ${testType === 'iq' ? 'IQ Test' : 'Mock Test'}`}
                        </button>
                    </div>
                </form>
            </div>
